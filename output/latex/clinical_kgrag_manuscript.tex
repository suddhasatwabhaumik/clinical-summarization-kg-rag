...
\begin{table}[H]
  ...
  \begin{tabular}{lcccc}
    ...
    \midrule
    Baseline Vector RAG & 512-Token Chunks (ClinicalBERT) & $0.78 \pm 0.05$ & $0.45 \pm 0.10$ & High allergy omissions, causal errors \\
    \textbf{Hybrid KG-RAG (Ours)} & Vector + 2-Hop Graph Edges & $\mathbf{0.81 \pm 0.03}$ & $\mathbf{0.12 \pm 0.04}$ & Zero allergy omissions, preserved timelines \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[H]
  ...
  \begin{tabular}{lcccc}
    \toprule
    \textbf{Embedding Model} & \textbf{Sample Size ($N$)} & \textbf{Mean Retrieval Precision} & \textbf{Precision SD} & \textbf{Tukey HSD Post-Hoc} \\
    \midrule
    Bio\_ClinicalBERT & 158 & 0.7400 & 0.0312 & Baseline \\
    BioLinkBERT & 158 & 0.8200 & 0.0245 & Diff vs. ClinicalBERT: $p < 0.01$ \\
    \textbf{PubMedBERT} & 158 & $\mathbf{0.8800}$ & 0.0198 & Diff vs. BioLinkBERT: $p < 0.05$ \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[H]
  ...
  \begin{tabular}{lccccc}
    \toprule
    \textbf{Prompting Strategy} & \textbf{ROUGE-1 F1} & \textbf{ROUGE-L F1} & \textbf{BERTScore F1} & \textbf{CREOLA CER} & \textbf{UMLS Entity F1} \\
    \midrule

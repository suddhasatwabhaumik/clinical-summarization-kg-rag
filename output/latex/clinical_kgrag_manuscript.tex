...
\subsection{Cloud Infrastructure, IaC Orchestration, and Reproducibility}
...
\begin{itemize}
  ...
  \item \textbf{Generative Inference}: Google Vertex AI hosting \texttt{gemini-1.5-pro} and \texttt{gemini-1.5-flash} endpoints with deterministic generation parameters ($T = 0.2, \text{top\_p} = 0.95$).
...
\begin{equation}
  \mathcal{R}_{\text{approved}} = \{\texttt{TREATS}, \texttt{CAUSES}, \texttt{HAS\_SYMPTOM}, \texttt{ASSOCIATED\_WITH}, \texttt{PREVENTS}, \texttt{DIAGNOSES}, \texttt{INDICATES}, \texttt{CONTRAINDICATES}, \texttt{IS\_A}\}
\end{equation}
...
\begin{equation}
  (T.\texttt{cui\_from} = S.\texttt{cui\_from} \land T.\texttt{cui\_to} = S.\texttt{cui\_to}) \lor (T.\texttt{cui\_from} = S.\texttt{cui\_to} \land T.\texttt{cui\_to} = S.\texttt{cui\_from})
\end{equation}
...
\begin{equation}
  \mathcal{N}_2(c_{\text{start}}) = \{ (c_{\text{start}}, r_1, v_1, r_2, v_2) \mid (c_{\text{start}}, v_1, r_1) \in \mathcal{N}_1 \land (v_1, v_2, r_2) \in \mathcal{E}, v_2 \neq c_{\text{start}} \}
\end{equation}
...
Rather than relying on unguided generation, the synthesized context is bound to a structured Graph-of-Thought (GoT) prompt template. The GoT prompt explicitly instructs the generative LLM (Google Vertex AI Gemini 1.5 Pro) to treat the injected knowledge graph triples as inviolable ground-truth constraints:
\begin{enumerate}
